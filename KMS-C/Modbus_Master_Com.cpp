...
namespace KMS
{
    namespace Modbus
    {
        ...
        unsigned int Master_Com::Request_A(Function aFunction, const void* aIn, unsigned int aInSize_byte, void* aOut, unsigned int aOutSize_byte)
        {
            ...
            if (lBuffer[2] > aOutSize_byte)
            {
                KMS_EXCEPTION_WITH_INFO(OUTPUT_TOO_SHORT, "Output buffer too short", lBuffer[1]);
            }
        ...
        void Master_Com::VerifyDeviceAddress(const uint8_t* aData)
        {
            ...
            if (GetDeviceAddress() != aData[0])
            {
                KMS_EXCEPTION_WITH_INFO(MODBUS_MASTER, "Invalid slave address", aData[0]);
            }
        }
