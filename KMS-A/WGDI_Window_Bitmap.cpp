...
namespace KMS
{
    namespace WGDI
    {
        ...
        void Window_Bitmap::SetBitmap(Graph::Bitmap* aBitmap)
        {
            assert(nullptr != aBitmap);

            assert(nullptr == mBitmap);
        ...
        unsigned int Window_Bitmap::OnStarting(void* aSender, void* aData)
        {
            assert(nullptr != mBitmap);
            ...
            auto lResult = Window::OnStarting(aSender, aData);
            if (CALLBACK_SUCCESS(lResult))
            {
                auto lInfo = reinterpret_cast<BITMAPINFO*>(mInternal);
